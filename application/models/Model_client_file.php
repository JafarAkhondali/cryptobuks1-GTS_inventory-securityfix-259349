...
class Model_client_file extends MY_Model {
	...
	public function count_all($q = null, $field = null)
	{
        ...
        if (empty($field)) {
	        foreach ($this->field_search as $field) {
	            if ($iterasi == 1) {
	                $where .= "pos_store_2_ibi_client_file.".$field . " LIKE '%" . $q . "%' ";
	            } else {
	                $where .= "OR " . "pos_store_2_ibi_client_file.".$field . " LIKE '%" . $q . "%' ";
	            }
	            $iterasi++;
	        }
	...
	public function get($q = null, $field = null, $limit = 0, $offset = 0, $select_field = [])
	{
        ...
        $this->db->order_by('pos_store_2_ibi_client_file.'.$this->primary_key, "DESC");
		$query = $this->db->get($this->table_name);
    ...
    function shuffle_code_file(){
    	    $randomString = '';
            $dateyear=date('Y');
            $lastid = $this->db->query("SELECT LPAD(MAX(INVOICE_NUMBER_CLIENT_FILE)+1,7,0) as Maxcounts FROM pos_store_2_ibi_client_file  WHERE YEAR(DATE_CREATION_CLIENT_FILE)='".$dateyear."'");
