
<script src="<?= BASE_ASSET; ?>/js/jquery.hotkeys.js"></script>
<script type="text/javascript">
//This page is a result of an autogenerated content made by running test.html with firefox.
function domo(){
 
   // Binding keys
   $('*').bind('keydown', 'Ctrl+e', function assets() {
      $('#btn_edit').trigger('click');
       return false;
   });

   $('*').bind('keydown', 'Ctrl+x', function assets() {
      $('#btn_back').trigger('click');
       return false;
   });
    
}


jQuery(document).ready(domo);
</script>
<!-- Content Header (Page header) -->
<section class="content-header">
   <h1>
      Articles      <small><?= cclang('detail', ['Articles']); ?> </small>
   </h1>
   <ol class="breadcrumb">
      <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
      <li class=""><a  href="<?= site_url('administrator/articles'); ?>">Articles</a></li>
      <li class="active"><?= cclang('detail'); ?></li>
   </ol>
</section>
<!-- Main content -->
<section class="content">
   <div class="row" >
     
      <div class="col-md-12">
         <div class="box box-warning">
            <div class="box-body ">

               <!-- Widget: user widget style 1 -->
               <div class="box box-widget widget-user-2">
                  <!-- Add the bg color to the header using any of the bg-* classes -->
                  <div class="widget-user-header ">
                    
                     <div class="widget-user-image">
                        <img class="img-circle" src="<?= BASE_ASSET; ?>/img/view.png" alt="User Avatar">
                     </div>
                     <!-- /.widget-user-image -->
                     <h3 class="widget-user-username">Articles</h3>
                     <h5 class="widget-user-desc">Detail Articles</h5>
                     <hr>
                  </div>

                 
                  <div class="form-horizontal" name="form_pos_store_2_ibi_articles" id="form_pos_store_2_ibi_articles" >
                   
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">ID ARTICLE </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->ID_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Nom Du Produit </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->DESIGN_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Emplacement </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->TITRE_RAYON); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Categorie </label>

                        <div class="col-sm-8">
                           <?php 
                                $user = $this->model_fournisseurs->get_user_info('pos_ibi_categories',$articles->ID_CATEGORIE,'ID_CATEGORIE');
                              if($user>0)
                                {

                                  foreach ($user as $value) 
                                    {
                                    
                                      echo "".$value->NOM_CATEGORIE;
                                    
                                    }
                                }
                             ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Sous Categorie </label>

                        <div class="col-sm-8">
                           <?php 
                                $user = $this->model_fournisseurs->get_user_info('pos_ibi_categories',$articles->REF_SOUS_CATEGORIE_ARTICLE,'ID_CATEGORIE');
                              if($user>0)
                                {

                                  foreach ($user as $value) 
                                    {
                                    
                                      echo "".$value->NOM_CATEGORIE;
                                    
                                    }
                                }
                             ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Part No </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->SKU_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Type Article </label>

                        <div class="col-sm-8">

                           <?php $type=$articles->TYPE_ARTICLE; 

                              if($type==1){
                                echo"Article physique";
                              }
                              else{
                                echo"Article numerique";
                              }

                           ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Etat </label>

                        <div class="col-sm-8">
                           
                           <?php $status=$articles->STATUS_ARTICLE; 
                              if($status==1){
                                echo "En vente";
                              }
                              else
                              {
                                echo"Disponible";
                              }

                           ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Gestion De Stock </label>

                        <div class="col-sm-8">
                           
                           <?php $stock_enabled=$articles->STOCK_ENABLED_ARTICLE; 
                              if($stock_enabled==1){
                                echo"Active";
                              }
                              else{
                                echo "Desactive";
                              }
                           ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Prix De Vente </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->PRIX_DE_VENTE_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Prix Fictif </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->SHADOW_PRICE_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Prix Promotionnel </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->PRIX_PROMOTIONEL_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Debut De La Promotion </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->SPECIAL_PRICE_START_DATE_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Fin De La Promotion </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->SPECIAL_PRICE_END_DATE_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Taille </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->TAILLE_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Unite </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->POIDS_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Couleur </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->COULEUR_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Hauteur </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->HAUTEUR_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Largeur </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->LARGEUR_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Description </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->DESCRIPTION_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label"> Apercu </label>
                        <div class="col-sm-8">
                             <?php if (is_image($articles->APERCU_ARTICLE)): ?>
                              <a class="fancybox" rel="group" href="<?= BASE_URL . 'uploads/pos_store_2_ibi_articles/' . $pos_store_2_ibi_articles->APERCU_ARTICLE; ?>">
                                <img src="<?= BASE_URL . 'uploads/pos_store_2_ibi_articles/' . $pos_store_2_ibi_articles->APERCU_ARTICLE; ?>" class="image-responsive" alt="image pos_store_2_ibi_articles" title="APERCU_ARTICLE pos_store_2_ibi_articles" width="40px">
                              </a>
                              <?php else: ?>
                              <label>
                                <a href="<?= BASE_URL . 'administrator/file/download/articles/' . $articles->APERCU_ARTICLE; ?>">
                                 <img src="<?= get_icon_file($articles->APERCU_ARTICLE); ?>" class="image-responsive" alt="image pos_store_2_ibi_articles" title="APERCU_ARTICLE <?= $articles->APERCU_ARTICLE; ?>" width="40px"> 
                               <?= $articles->APERCU_ARTICLE ?>
                               </a>
                               </label>
                              <?php endif; ?>
                        </div>
                    </div>
                                       
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Date de creation de l'article </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->DATE_CREATION_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Date de modification de l'article </label>

                        <div class="col-sm-8">
                           <?= _ent($articles->DATE_MOD_ARTICLE); ?>
                        </div>
                    </div>
                                         
                    <div class="form-group ">
                        <label for="content" class="col-sm-2 control-label">Auteur </label>

                        <div class="col-sm-8">
                           
                           <?php 
                                $user = $this->model_fournisseurs->get_user_info('aauth_users',$articles->AUTHOR_ARTICLE,'id');
                                foreach ($user as $value) {
                                  echo "".$value->username;
                                }
                               ?>
                        </div>
                    </div>
                                        
                    <br>
                    <br>

                    <div class="view-nav">
                        <?php is_allowed('pos_store_2_ibi_articles_update', function() use ($articles){?>
                        <a class="btn btn-flat btn-info btn_edit btn_action" id="btn_edit" data-stype='back' title="edit pos_store_2_ibi_articles (Ctrl+e)" href="<?= site_url('administrator/articles/edit/'.$articles->ID_ARTICLE); ?>"><i class="fa fa-edit" ></i> <?= cclang('update', ['Pos Store 2 Ibi Articles']); ?> </a>
                        <?php }) ?>
                        <a class="btn btn-flat btn-default btn_action" id="btn_back" title="back (Ctrl+x)" href="<?= site_url('administrator/articles/'); ?>"><i class="fa fa-undo" ></i> <?= cclang('go_list_button', ['Pos Store 2 Ibi Articles']); ?></a>
                     </div>
                    
                  </div>
               </div>
            </div>
            <!--/box body -->
         </div>
         <!--/box -->

      </div>
   </div>
</section>
<!-- /.content -->
